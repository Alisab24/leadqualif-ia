...
// ── Helper WhatsApp (même logique que Dashboard) ────────────────────────
const toWhatsAppPhone = (raw = '') => {
  const digits = (raw || '').replace(/\D/g, '')
  if (!digits) return ''
  // Déjà international (commence par indicatif pays)
  if (!digits.startsWith('0')) return digits
  // FR/BE/CH local (10 chiffres, 0X…) → 33 + 9 chiffres
  if (digits.length === 10) return '33' + digits.substring(1)
  // UK local (11 chiffres, 0X…) → 44 + 10 chiffres
  if (digits.length === 11) return '44' + digits.substring(1)
  return digits
}
...
export default function ScraperPage() {
  ...
  const injectLead = async (lead) => {
    ...
    // Normaliser au format international pour WhatsApp (+33XXXXXXXXX)
    const rawTel = lead.telephone || lead.telephone_secondaire || null
    const telephoneFinal = rawTel
      ? (buildWAUrl(rawTel) ? '+' + toWhatsAppPhone(rawTel) : rawTel)
      : null
                          ...
                          <td className="px-3 py-2.5 overflow-hidden">
                            {lead.telephone ? (
                              <div className="flex items-center gap-1.5">
                                <span className="text-xs text-slate-600 truncate">{lead.telephone}</span>
                                <a
                                  href={buildWAUrl(lead.telephone) || '#'}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  onClick={e => { if (!buildWAUrl(lead.telephone)) e.preventDefault() }}
