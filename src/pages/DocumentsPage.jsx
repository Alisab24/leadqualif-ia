...
const DocumentsPage = () => {
  ...
  const [signingDoc,  setSigningDoc]        = useState(null); // docId en cours d'envoi signature
  ...
  const handleSendForSignature = async (doc) => {
    ...
    if (!window.confirm(`Envoyer "${doc.titre || doc.reference}" à ${doc.client_email} pour signature électronique ?`)) return;
                    ...
                    {(() => {
                      ...
                      <tr key={doc.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-3 py-3.5 overflow-hidden">
                          ...
                          {doc.signature_status === 'awaiting' && (
                            <span className="ml-1 inline-flex px-1.5 py-0.5 text-[10px] font-semibold rounded-full bg-amber-100 text-amber-700" title="En attente de signature">
                              ✍️ Attente
                            </span>
                          )}
                          {doc.signature_status === 'completed' && (
                            <span className="ml-1 inline-flex px-1.5 py-0.5 text-[10px] font-semibold rounded-full bg-teal-100 text-teal-700" title="Document signé électroniquement">
                              ✅ Signé
                            </span>
                          )}
                        </td>
                        ...
                        <td className="px-3 py-3.5 text-right overflow-hidden">
                          <div className="flex items-center justify-end gap-1 flex-nowrap">
                            ...
                            {/* Signature électronique — devis/contrats/mandats */}
                            {['devis','contrat','mandat'].includes(doc.type) && doc.signature_status !== 'completed' && (
                              <button
                                onClick={() => handleSendForSignature(doc)}
                                disabled={signingDoc === doc.id || doc.signature_status === 'awaiting'}
                                className="p-1.5 text-teal-500 hover:text-teal-700 hover:bg-teal-50 rounded-lg transition-colors disabled:opacity-40"
                                title={doc.signature_status === 'awaiting'
                                  ? 'En attente de signature client'
                                  : `Envoyer pour signature électronique à ${doc.client_email || 'client'}`}
                              >{signingDoc === doc.id ? '⏳' : '✍️'}</button>
                            )}
