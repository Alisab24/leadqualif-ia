 ...
 * Ouvre le portail de facturation Stripe (gérer abonnement, factures, annulation)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (customers.data.length === 0) {
      return res.status(404).json({ error: 'Aucun compte Stripe trouvé. Abonnez-vous d\'abord.' });
    }

    const portalSession = await stripe.billingPortal.sessions.create({
      customer: customers.data[0].id,
      return_url: returnUrl || 'https://www.leadqualif.com/settings?tab=facturation',
    });
